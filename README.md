...
7. Save as .csv (referenced as demultiplex.csv in subsequent instructions). Example:
...
4. For the first time you are using the script, you will need to alter the permissions on the file:
   a. In your scripts directory run:
   ```chmod 755 GTseq_BarcodeSplit.KML.py```
   ```chmod 755 queue_demultiplex.sh```
...
8. Edit queue_demultiplex.sh:
   a. ```nano queue_demultiplex.sh```
      i. Edit line 2 for the appropriate partition
      ii. Edit line 6 with your email
      iii. Edit all paths in Lines 14-17
      iv. If needed edit Line 20 to direct to your scripts directory
   b. Hit Ctrl X -> Y -> enter to save
...
10. For reference, this step has taken ~45 minutes to demultiplex a NovaSeq 6000 run with 4 plates
...
3. This will take a while (about ~1.5 hours). If you are feeling ambitious, you could write some code to zip the files in parallel, but I haven't done that yet. Once/if I do, I'll update this section.
